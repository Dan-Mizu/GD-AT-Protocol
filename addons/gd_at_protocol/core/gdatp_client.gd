...
func xrpc_post(path: String, body_dict: Dictionary, require_auth: bool = false) -> Dictionary:
	...
	if response_code < 200 or response_code >= 300:
		var err_body: Variant = null
		if not text.is_empty():
			err_body = JSON.parse_string(text)
...
func http_get_json(url: String) -> Dictionary:
	...
	if err != OK:
		return {
			"ok": false,
			"error": "RequestStartFailed",
			"message": "Failed to start HTTP GET",
			"godot_error": err,
		}
...
func http_post_json(url: String, body_dict: Dictionary, require_auth: bool = false) -> Dictionary:
	var json_text := JSON.stringify(body_dict)
	var headers: PackedStringArray = [ "Content-Type: application/json" ]
